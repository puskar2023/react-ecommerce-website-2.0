...
const CartItem = ({ item }) => {
  ...
  rounded-xl shadow-2xl border-gray-800 transition duration-300 hover:border-green-600/50"
    >
      ...
      <div className="flex items-center justify-between sm:justify-end w-full sm:w-2/5 sm:mt-0 space-x-4">
        ...
        <p className="font-extrabold text-green-300 w-24 text-right hidden md:block">
          ₹{(item.price * item.quantity).toFixed(2)}
        </p>
        <button onClick={()=>removeFromCart(item.id, true)}
        className="p-3 bg-green-800/20 text-store-200 rounded-full hover:bg-green-800/40 transition duration-150 shadow-md"
          >
          <X className="w-5 h-5" />
        </button>
      </div>
